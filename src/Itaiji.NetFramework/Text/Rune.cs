...
#pragma warning restore SA1001
#endif
    {
        ...
        private static byte[] AsciiCharInfo =>
        [
        ...
        private static Rune ChangeCaseCultureAware(Rune rune, CultureInfo culture, bool toUpper)
        {
            ...
            var modified = toUpper ? original.ToUpper(culture) : original.ToLower(culture);

            return Rune.GetRuneAt(modified, 0);

            //char[] original = new char[MaxUtf16CharsPerRune]; // worst case scenario = 2 code units (for a surrogate pair)
            //char[] modified = new char[MaxUtf16CharsPerRune]; // case change should preserve UTF-16 code unit count
        ...
        /// </remarks>
        public static OperationStatus DecodeFromUtf16(char[] source, out Rune result, out int charsConsumed)
        {
            if (source.Length != 0)
            {
        ...
        /// invalid sequences while iterating through the loop.
        /// </remarks>
        public static OperationStatus DecodeFromUtf8(byte[] source, out Rune result, out int bytesConsumed)
        {
        ...
        /// </summary>
        /// <remarks>
        /// This method is very similar to <see cref="DecodeFromUtf16(char[], out Rune, out int)"/>, but it allows
        ...
        /// </remarks>
        public static OperationStatus DecodeLastFromUtf16(char[] source, out Rune result, out int charsConsumed)
        {
        ...
        /// </summary>
        /// <remarks>
        /// This method is very similar to <see cref="DecodeFromUtf8(byte[], out Rune, out int)"/>, but it allows
        /// the caller to loop backward instead of forward. The typical calling convention is that on each iteration
        /// of the loop, the caller should slice off the final <paramref name="bytesConsumed"/> elements of
        /// the <paramref name="source"/> buffer.
        /// </remarks>
        public static OperationStatus DecodeLastFromUtf8(byte[] source, out Rune value, out int bytesConsumed)
        {
            int index = source.Length - 1;
            if ((uint)index < (uint)source.Length)
            {
                ...
                if (((byte)tempValue & 0x40) != 0)
                {
                    ...
                    var newSource2 = new byte[source.Length - index];
                    Array.Copy(source, index, newSource2, 0, newSource2.Length);
                    return DecodeFromUtf8(newSource2, out value, out bytesConsumed);
                }
            ...
            ForwardDecode:
                ...
                var newSource = new byte[source.Length - index];
                Array.Copy(source, index, newSource, 0, newSource.Length);
                Debug.Assert(newSource.Length != 0, "Shouldn't reach this for empty inputs.");

                OperationStatus operationStatus = DecodeFromUtf8(newSource, out Rune tempRune, out int tempBytesConsumed);
                if (tempBytesConsumed == newSource.Length)
                {
        ...
        /// </exception>
        public int EncodeToUtf16(char[] destination)
        {
        ...
        /// If <paramref name="destination"/> is not large enough to hold the output.
        /// </exception>
        public int EncodeToUtf8(byte[] destination)
        {
        ...
        internal static int ReadFirstRuneFromUtf16Buffer(char[] input)
        {
            if (input.Length == 0)
            {
                return -1;
        ...
        public override string ToString()
        {
            ...
            {
                char[] buffer = new char[MaxUtf16CharsPerRune];
        ...
        private static bool TryEncodeToUtf8(Rune value, byte[] destination, out int bytesWritten)
        {
